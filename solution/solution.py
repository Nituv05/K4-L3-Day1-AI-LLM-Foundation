...
def run_assistant(
    ...
    while True:
        ...
        cost_info = estimate_cost(user_msg, reply, OPENAI_MODEL)
...
def format_comparison_table(results: list[dict]) -> str:
    ...
    def shorten(value: Any, width: int = 40) -> str:
        ...
        return text if len(text) <= width else text[:37] + "..."
    ...
    for result in results:
        rows.append([
            ...
            f'{result.get("gpt4o_latency", 0.0):.3f}',
            f'{result.get("mini_latency", 0.0):.3f}',
        ])
